...
void naive_convbase(int dim,int *src, int *ker,int *dest) {
    ...
    for(j = 0; j < dim-8+1; j++){
        ...
        for(i = 0; i < dim-8+1; i++){
                ...
                dest[j*dim+i]=0;
                for(k = 0; k < 8; k++){
                    sum += src[(j+l)*dim+(i+k)]*ker[l*dim+k];
                }
                dest[j*dim+i] += sum;
            }
...
char naive_convijlk_descr[] = "naive_conv: Naive baseline implementation jlik";
...
char naive_convijlkcrazyunroll_descr[] = "naive_conv: Naive baseline implementation jlik crazy unroll";
