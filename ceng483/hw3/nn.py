...
def mse_loss(a,b):
    a = a.data.numpy()
    b = b.data.numpy()
    assert a.shape==b.shape, "Unmatching shapes"
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.upsample = nn.Upsample(scale_factor=2, mode="bilinear",align_corners=False) #in paper they use mode='nearest'
...
train_dset = ImageDataset(type="train")
val_dset = ImageDataset(type="val")
...
def toRgb256(L,ab):
    ...
    ab = np.transpose(ab,(0,2,3,1)).astype(np.float64)
...
def train(net):
    ...
    for epoch in range(param["epoch"]):
        ...
        for batch_idx, (gray_L, rgb_256_ab) in enumerate(train_loader):
            
            gray_L,rgb_256_ab = Variable(gray_L).float() ,Variable(rgb_256_ab).float()
            
            if go_cuda:
                gray_L = gray_L.cuda(SELECT_GPU)
                rgb_256_ab = rgb_256_ab.cuda(SELECT_GPU)
                
            #resize
            optimizer.zero_grad()
            
            pred_rgb_256_ab = net(gray_L)
            
            loss = criterion(pred_rgb_256_ab,rgb_256_ab)
        ...
        for index, (gray_L, rgb_256_ab) in enumerate(acc_val_loader):
            gray_L,rgb_256_ab = Variable(gray_L).float(),Variable(rgb_256_ab).float()
            pred_rgb_256_ab = net(gray_L)
            assert index<1, "This loop should not run more than once"

        val_loss = float(mse_loss(pred_rgb_256_ab,rgb_256_ab))

        pred_rgb_256 = toRgb256(gray_L,pred_rgb_256_ab)
